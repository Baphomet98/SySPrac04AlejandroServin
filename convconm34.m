...
function convconmE(x,h)
  figure (1) % Se crea una figura para hacer las gráficas
  dtau = .005; % Base de los rectangulos para realizar la integral 
  ...
  for t = tvec, % Cantidad de traslaciones
      ...
      axis ([tau(1) tau(end) -2.0 2]); % límites de los ejes
      ...
      axis ([tau(1) tau(end) -2.0 4.5]); % límites del eje
      grid; % malla
      drawnow; % efecto de movimiento continuo
      F(ti)=getframe(gcf);
  end
  
  video=VideoWriter('VIDEOp6.avi','Motion JPEG AVI');
  video.FrameRate=60;
  open(video)
  writeVideo(video,F);
  close(video)
